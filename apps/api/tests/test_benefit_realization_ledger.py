...
class FakeBenefitLedgerRepo:
    ...
    def create_benefit_ledger_entry(
        self, initiative_id: str, data: dict[str, object]
    ) -> dict[str, object]:
        row = {
            **data,
    ...
    def delete_benefit_ledger_entry(self, initiative_id: str, entry_id: str) -> None:
        self.entries = [
            row
            for row in self.entries
            if not (row["initiative_id"] == initiative_id and row["id"] == entry_id)
        ]
    ...
    repo = FakeBenefitLedgerRepo(
        ...
        latest_plan={
            ...
            "snapshot": {
                ...
                "summary": {"net_value_plan": "0.0000", "net_value_actual": None},
            },
        },
        entries=[
            {
...
def test_benefit_ledger_summary_rolls_up_weekly_monthly_and_yearly(
    benefit_service: FinancialService,
) -> None:
...
def test_benefit_ledger_empty_summary_returns_zeroed_response() -> None:
    service = cast(Any, FinancialService.__new__(FinancialService))
    service._tenant_id = TENANT_ID
    service._repo = FakeBenefitLedgerRepo(
        initiative_ids={INITIATIVE_ID}, latest_plan=None, entries=[]
    )
    service._ensure_tenant_initiative = lambda initiative_id: None
...
def test_benefit_ledger_entry_crud_routes(
    monkeypatch: pytest.MonkeyPatch, benefit_service: FinancialService
) -> None:
