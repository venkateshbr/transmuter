...
def suggest_kpis(
    ...
    patterns = _kpi_patterns_for_type(initiative_type, value_text)
    suggestions = [
        KPISuggestion(
            name=pattern["name"],
            type=pattern["type"],
    ...
    return KPISuggestionResult(suggestions=suggestions)
...
def scan_risk_patterns(initiative_draft: InitiativeDraft) -> RiskPatternScanResult:
    ...
    patterns = _risk_patterns_for_type(initiative_draft.type)
    risks = [
        RiskPattern(
    ...
    if initiative_draft.dependencies and not any(r.type == "operational" for r in risks):
        risks.append(
            RiskPattern(
...
def _kpi_patterns_for_type(
    ...
    if "margin" in value_logic and not any(k["name"] == "Gross margin uplift" for k in selected):
        selected = [
            {
...
def _risk_patterns_for_type(initiative_type: str | None) -> list[dict[str, str]]:
    ...
    by_type: dict[str, list[dict[str, str]]] = {
        ...
        "cost_avoidance": [
            {
                "description": "Avoided-cost baseline may be challenged for {name}",
                "type": "financial",
                "impact": "medium",
                "likelihood": "high",
                "mitigation": "Document the counterfactual baseline and obtain Finance approval.",
                "rationale": "Cost avoidance requires stronger baseline governance than booked savings.",
            },
            {
                "description": "Demand may reappear in another channel after {name}",
                "type": "operational",
                "impact": "medium",
                "likelihood": "medium",
                "mitigation": "Monitor demand displacement and adjust controls across channels.",
                "rationale": "Avoidance work often shifts cost instead of eliminating exposure.",
            },
        ],
        "compliance": [
            {
...
def _masked_free_text(text: str) -> str:
    ...
    masked = re.sub(r"\+?\d[\d\s().-]{7,}\d", _mask_phone_like_value, masked)
    return masked
...
def _extract_type(lowered: str) -> str | None:
    patterns = (
        ("revenue_growth", ("revenue growth", "sales growth", "commercial growth", "uplift revenue")),
