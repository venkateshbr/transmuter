...
def extract_action_items(
    ...
    for chunk in chunks:
        ...
        if not any(
            word in lowered
            for word in ("action", "todo", "to do", "next step", "will", "should", "owns", "please")
        ):
            continue
...
def extract_meeting_decisions(
    ...
    for chunk in chunks:
        ...
        if any(
            phrase in lowered
            for phrase in ("decided", "decision", "agreed", "approved", "sign off", "signed off")
        ):
            decisions.append(
                MeetingDecision(
            ...
            if initiative or linked_initiatives:
                updates.append(
                    InitiativeStatusSignal(
                        ...
                        if initiative
                        else linked_initiatives[0].name,
                        summary=text[:2000],
