...
class FinancialService:
    ...
    def list_benefit_ledger_entries(self, initiative_id: str) -> list[BenefitLedgerEntry]:
        self._ensure_tenant_initiative(initiative_id)
        return [self._to_benefit_ledger_entry(row) for row in self._repo.list_benefit_ledger_entries(initiative_id)]
    ...
    def update_benefit_ledger_entry(
        ...
        if not row:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Benefit ledger entry not found")
        return self._to_benefit_ledger_entry(row)
    ...
    def _benefit_ledger_period_summaries(
        ...
        for row in rows:
            ...
            bucket["bankable_plan_amount"] = _dec(bucket["bankable_plan_amount"]) + _dec(row.get("bankable_plan_amount"))
            bucket["actual_amount"] = _dec(bucket["actual_amount"]) + _dec(row.get("actual_amount"))

        periods: list[BenefitLedgerPeriodSummary] = []
        for year, week, month in sorted(grouped, key=lambda item: self._benefit_period_sort(item, granularity)):
    ...
    def _has_any_actual_signal(
        ...
        for row in entries:
            if any(_dec(row.get(field)) != Decimal("0") for field in ("revenue_uplift_actual", "gross_margin_actual", "gm_uplift_actual", "cogs_actual")):
                return True
        for row in costs:
    ...
    def _has_multi_scenario_signal(
        ...
        *,
        ...
        if config:
            return any(
                item.is_active and item.item_type == "metric" and item.system_metric_key in {"revenue_uplift_base", "revenue_uplift_high", "gross_margin_base", "gross_margin_high"}
