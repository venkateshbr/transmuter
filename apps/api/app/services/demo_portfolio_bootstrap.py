...
def _financial_row(initiative: InitiativeSeed, year: int, quarter: int) -> dict[str, str | int | None]:
    multiplier = _quarter_multiplier(initiative, year, quarter)
    revenue_base = initiative.base_value * multiplier if initiative.kind in {"revenue_growth", "automation"} else Decimal("0")
    revenue_high = initiative.high_value * multiplier if initiative.kind in {"revenue_growth", "automation"} else Decimal("0")
    actual_available = initiative.actual_start is not None and date(year, *_quarter_date(year, quarter)[0].timetuple()[1:3]) >= initiative.actual_start
    actual_factor = Decimal("0.96") if initiative.kind == "revenue_growth" else Decimal("0.93") if initiative.kind == "cost_reduction" else Decimal("0.90")
...
def _seed_initiatives(
    ...
    for initiative in INITIATIVES:
        ...
        payload = {
            ...
            "actual_start": initiative.actual_start.isoformat() if initiative.actual_start else None,
            "actual_end": None,
...
def _seed_milestones(
    ...
    for index, initiative in enumerate(INITIATIVES):
        ...
        for sort_order, (name, planned_start, planned_end, actual_start, actual_end, status) in enumerate(milestone_specs):
            existing = (
            ...
            if existing.data:
                client.table("milestones").update(payload).eq("id", existing.data[0]["id"]).execute()
            else:
    ...
    for initiative in INITIATIVES:
        ...
        for year in (2026, 2027, 2028):
            for quarter in (1, 2, 3, 4):
                ...
                if existing.data:
                    client.table("financial_entries").update(payload).eq("id", existing.data[0]["id"]).execute()
                else:
                    client.table("financial_entries").insert({"id": str(uuid4()), **payload}).execute()


def _seed_cost_lines(client: Client, tenant_id: str, initiative_ids: dict[str, str]) -> None:
    cost_lines = [
        ("erp", "ERP Licence & Implementation", "implementation", 2026, 1, Decimal("250000"), Decimal("260000"), False),
        ("offshoring", "Shared Service Transition", "implementation", 2026, 3, Decimal("180000"), Decimal("175000"), False),
        ("procurement", "Vendor Renegotiation Workstream", "operating", 2026, 4, Decimal("90000"), Decimal("85000"), True),
        ("ai_service_desk", "AI Knowledge Base Build", "implementation", 2027, 2, Decimal("120000"), Decimal("110000"), False),
    ]
    for label, name, category_key, year, quarter, amount_plan, amount_actual, recurring in cost_lines:
        ...
        if existing.data:
            client.table("financial_cost_lines").update(payload).eq("id", existing.data[0]["id"]).execute()
        else:
...
def _seed_dependencies(
    ...
    dependencies = [
        ...
        ("pmo", "rev_asia", "enables", "active", "medium", "TRN-005 depends on benefits governance."),
        ("erp", "offshoring", "blocks", "blocking", "high", "ERP stabilisation is required before offshoring scale-up."),
        ("compliance", "supply_chain", "requires_decision", "at_risk", "high", "Privacy controls must be final before the control tower can scale."),
        ("productivity", "ai_service_desk", "informs", "active", "low", "Productivity rollout improves adoption of the AI desk."),
        ("procurement", "pricing", "enables", "active", "medium", "Savings and pricing governance reinforce each other."),
        ("offshoring", "procurement", "blocks", "at_risk", "high", "Shared-services model influences procurement execution.") ,
    ]
    for upstream_label, downstream_label, dep_type, status, severity, note in dependencies:
        ...
        if existing.data:
            client.table("initiative_dependencies").update(payload).eq("id", existing.data[0]["id"]).execute()
        else:
            client.table("initiative_dependencies").insert({"id": str(uuid4()), **payload}).execute()
...
def _seed_benefit_ledger(client: Client, tenant_id: str, initiative_ids: dict[str, str]) -> None:
    ...
    for label in ledger_initiatives:
        ...
        for month_index in range(1, 13):
            ...
            if existing.data:
                client.table("benefit_realization_ledger").update(payload).eq("id", existing.data[0]["id"]).execute()
            else:
                client.table("benefit_realization_ledger").insert({"id": str(uuid4()), **payload}).execute()


def _seed_initiative_controls(client: Client, tenant_id: str, initiative_ids: dict[str, str]) -> None:
    controls = {
        ...
        "ai_service_desk": ("68.00", "partially_realized", "Knowledge automation adoption is ramping."),
    }
