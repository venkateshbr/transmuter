...
def _financials_summary(
    ...
    period_entries = [
        item for item in entries if _financial_row_in_period(item, period_start, period_end)
    ]
    period_costs = [
        item for item in cost_lines if _financial_row_in_period(item, period_start, period_end)
    ]
    return FinancialsContextSummary(
        revenue_plan=_money(
            sum((_dec(item.get("revenue_uplift_base")) for item in period_entries), Decimal("0"))
        ),
        revenue_actual=_money(
            sum((_dec(item.get("revenue_uplift_actual")) for item in period_entries), Decimal("0"))
        ),
        costs_plan=_money(
            sum((_dec(item.get("amount_plan")) for item in period_costs), Decimal("0"))
        ),
        costs_actual=_money(
            sum((_dec(item.get("amount_actual")) for item in period_costs), Decimal("0"))
        ),
    )
