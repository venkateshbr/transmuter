  ...
  template: `
    <div class="min-h-screen p-8 space-y-8" style="background:var(--t-bg)">
      <header class="flex flex-wrap items-end justify-between gap-5 border-b border-[var(--t-border)] pb-6">
        <div>
          <p class="text-[10px] font-black uppercase tracking-widest text-[var(--t-accent)]">Benefits Ledger</p>
          <h1 class="mt-2 text-3xl font-black text-[var(--t-text-primary)]">
            Benefit Tracking<span class="text-[var(--t-blue-light)]">.</span>
          </h1>
          <p class="mt-2 max-w-3xl text-sm leading-6 text-[var(--t-text-secondary)]">
            Switch between weekly, monthly, and yearly ledger views to reconcile plan line performance against the locked bankable plan.
          </p>
        </div>
      ...
      <section class="grid gap-4 lg:grid-cols-[1.4fr_0.6fr]">
        <div class="card p-5">
          ...
          <div class="mt-6 grid gap-4 md:grid-cols-4">
            <div class="border border-[var(--t-border)] bg-[var(--t-surface-raised)] p-4">
              ...
              <p class="mt-2 text-lg font-black text-[var(--t-text-primary)]">
                {{ summary().locked_bankable_plan_version ? 'v' + summary().locked_bankable_plan_version : 'Unlocked' }}
              </p>
              <p class="mt-2 text-xs font-bold text-[var(--t-text-secondary)]">
                {{ summary().locked_bankable_plan_version ? 'Read-only plan line' : 'Entries remain editable' }}
              </p>
            </div>
            <div class="border border-[var(--t-border)] bg-[var(--t-surface-raised)] p-4">
        ...
        <aside class="space-y-4">
          <div class="card border-l-4 border-[var(--t-accent)] p-5">
            <p class="text-[9px] font-black uppercase tracking-widest text-[var(--t-text-tertiary)]">Editability</p>
            <p class="mt-2 text-lg font-black text-[var(--t-text-primary)]">Ledger rows are editable</p>
            <p class="mt-2 text-sm leading-6 text-[var(--t-text-secondary)]">
              The bankable plan line is locked to the latest approved version. Benefit entries can still be adjusted in the ledger workflow, while the snapshot version stays fixed for comparison.
            </p>
          </div>

          <div class="card p-5">
            <p class="text-[9px] font-black uppercase tracking-widest text-[var(--t-text-tertiary)]">Period scope</p>
            <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div class="border border-[var(--t-border)] p-3">
                <p class="text-[9px] font-black uppercase tracking-widest text-[var(--t-text-tertiary)]">Current granularity</p>
                <p class="mt-1 text-lg font-black text-[var(--t-text-primary)] capitalize">{{ granularity() }}</p>
              </div>
              <div class="border border-[var(--t-border)] p-3">
                <p class="text-[9px] font-black uppercase tracking-widest text-[var(--t-text-tertiary)]">Periods</p>
                <p class="mt-1 text-lg font-black text-[var(--t-text-primary)]">{{ summary().periods.length }}</p>
              </div>
            </div>
          </div>
        </aside>
...
export class BenefitTrackingComponent implements OnInit {
  ...
  readonly granularity = signal<FinancialLedgerGranularity>('weekly');
  readonly summary = signal<BenefitLedgerSummaryResponse>({
    initiative_id: '',
    granularity: 'weekly',
    locked_bankable_plan_version: null,
    periods: [],
