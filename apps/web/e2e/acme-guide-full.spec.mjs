  ...
  try {
    ...
    if (meetingName) {
      ...
      if (await meeting.count()) {
        await meeting.check();
        await recoveryPage
          .getByLabel('Meeting cleanup confirmation phrase')
          .fill('DELETE MEETINGS');
    ...
    if (initiativeName && initiativeCode) {
      await openRoute(recoveryPage, '/admin');
      await recoveryPage.getByRole('button', { name: 'Open Data Cleanup admin tab' }).click();
      const initiative = recoveryPage.getByRole('button', {
        name: `Select ${initiativeName} for deletion`,
      });
...
test('ACME transformation-office client demo guide in one headed browser session', async ({
  page,
  browser,
}) => {
  ...
  try {
    await scenario(
      ...
      'Establish that the client demo uses the healthy public dev app and a real ACME transformation-office identity.',
      async () => {
    ...
    await scenario(
      ...
      'Confirm the presenter is operating inside ACME and has the transformation-office management role.',
      async () => {
    ...
    await scenario(
      ...
      'Show that all prerequisite structure is configured before initiatives are governed.',
      async () => {
    ...
    await scenario(
      ...
      'Explain the workstreams, business units, markets, theme, and tags used to slice the ACME portfolio.',
      async () => {
        ...
        expect(await inputValues(page.getByLabel('Workstream name', { exact: true }))).toEqual([
          'Automation',
          'Commercial Growth',
          'ERP & Data Platform',
        ...
        expect(await inputValues(page.getByLabel('Business unit name', { exact: true }))).toEqual([
          'Commercial',
          'Corporate',
          'Operations',
          'Shared Services',
        ...
        expect(await inputValues(page.getByLabel('Market name', { exact: true }))).toEqual([
          'Group',
          'Regional',
        ]);
        expect(await inputValues(page.getByLabel('Theme name', { exact: true }))).toEqual([
          'Manufacturing productivity and profitable growth',
        ]);
        expect(await inputValues(page.getByLabel('Tag name', { exact: true }))).toEqual([
          'automation',
          'offshoring',
          'commercial',
          'other',
    ...
    await scenario(
      ...
      'Show the common currency, fiscal calendar, original baseline, scenarios, value bridge, and cost taxonomy behind comparable business cases.',
      async () => {
        ...
        await expect(
          page.getByLabel('Fiscal year start month').locator('option:checked'),
        ).toHaveText('January');
        ...
        await expect(
          page.getByLabel('Tenant annual baseline for Annual Revenue Baseline'),
        ).toHaveValue('20000000.0000');
        await expect(
          page.getByLabel('Tenant annual baseline for Annual Gross Margin Baseline'),
        ).toHaveValue('9000000.0000');
        expect(await inputValues(page.getByLabel('Scenario label'))).toEqual([
          'Baseline',
          'Plan Base',
          'Plan High',
          'Actual',
        ...
        await expect(page.getByLabel('Metric formula key')).toBeDisabled();
        ...
        expect(await inputValues(page.getByLabel('Cost category label'))).toEqual(
          expect.arrayContaining(['Software / Licenses', 'People Support']),
        );
      },
    );
    ...
    await scenario(
      ...
      'Explain stage-gate control and role separation before demonstrating delivery workflows.',
      async () => {
        ...
        expect(await inputValues(page.getByLabel('Gate number', { exact: true }))).toEqual([
          '1',
          '2',
          '3',
          '4',
        ...
        expect(await inputValues(page.getByLabel('Approver roles'))).toEqual([
        ...
        for (let index = 0; index < 5; index += 1)
          await expect(requireAllCriteria.nth(index)).toBeChecked();
    ...
    await scenario(
      'People roster',
      'Demonstrate the seeded operating team and the browser-visible invite/access controls.',
      async () => {
        await openRoute(page, '/people');
        await expect(page.getByRole('button', { name: 'Add User' })).toBeVisible();
        await expect(
          page.locator('.card').filter({ has: page.getByRole('button', { name: 'Profile' }) }),
        ).toHaveCount(10);
    ...
    await scenario(
      'Executive dashboard',
      'Open with the portfolio story: ten initiatives, value matrix, stage waterline, actions, KPI pulse, and recent activity.',
      async () => {
        await openRoute(page, '/dashboard');
        await expect(page.getByTestId('dashboard-total-initiatives')).toContainText('10');
        for (const id of [
          'dashboard-workstream-targets',
          'dashboard-stage-gate-waterline',
          'dashboard-value-matrix',
    ...
    await scenario(
      'Initiative pipeline and filters',
      'Show how a transformation manager narrows the portfolio by strategic tag and workstream without losing the ten-initiative master view.',
      async () => {
        ...
        await expect(page.locator('[data-testid="initiatives-filter-workstream"]')).toContainText(
          '1',
        );
        await page.getByTestId('initiatives-clear-filters').click();
      },
    ...
    await scenario(
      ...
      'Explain FY28 run-rate benefits, recurring cost, net value, actuals, category drilldown, and contributor traceability.',
      async () => {
        ...
        if ((await actuals.isEnabled()) && (await actuals.getAttribute('aria-pressed')) === 'false')
          await actuals.click();
        ...
        await page
          .getByLabel('Filter cost category')
          .selectOption({ label: 'Software / Licenses' });
        ...
        const contributorButton = page
          .getByRole('button', { name: /Show initiatives for 2028-M01/i })
          .first();
    ...
    await scenario(
      'Initiative value case and locked baseline',
      'Trace portfolio value into the ACME aftermarket-growth initiative and explain why its approved original baseline is immutable.',
      async () => {
        await openRoute(page, '/initiatives/pipeline');
        ...
        await expect(
          page.getByLabel('Initiative baseline Annual Gross Margin Baseline'),
        ).toBeDisabled();
        await expect(
          page.getByRole('button', { name: 'Save initiative annual baseline' }),
        ).toBeDisabled();
        await expect(page.getByText(/locked by governance/i)).toBeVisible();
        await expect(
          page.getByRole('link', { name: /Configure initiative financial metrics/i }),
        ).toBeVisible();
    ...
    await scenario(
      'Bankable plan and benefits governance',
      'Connect the approved value case to rebaseline history, finance validation, and realized benefit tracking.',
      async () => {
        await openRoute(page, '/financials/bankable-plan');
        await selectOptionContaining(
          page.getByLabel('Select initiative for bankable plan review'),
          'ENT-005',
        );
        ...
        await expect(
          page.getByRole('heading', { name: /Locked Baseline Realization/i }),
        ).toBeVisible();
    ...
    await scenario(
      'Waterline and initiative portfolio',
      'Show selection discipline and reconcile original baselines to target-year value without locking a new snapshot during the demo.',
      async () => {
    ...
    await scenario(
      ...
      'Explain how enterprise platform, PMO, change, and advisory costs are allocated separately from direct initiative economics.',
      async () => {
        await openRoute(page, '/shared-costs');
        for (const pool of [
          'Group technology and data platform',
          'Transformation PMO and benefits office',
          'Shared change and training support',
          'Central advisory and vendor support',
        ]) {
    ...
    await scenario(
      'Progress, PMO, and control tower',
      'Demonstrate delivery oversight across milestones, status, actions, governance, risks, KPIs, allocated cost, and net value.',
      async () => {
        const routes = [
          ['/progress', /Progress|Delivery/i],
    ...
    await scenario(
      'Guided initiative authoring and HITL',
      'Create a temporary client example, review AI suggestions before write, and prove the result appears in the real portfolio.',
      async () => {
        ...
        await page
          ...
          .fill(
            'Recover aftermarket margin by standardizing price corridors and managing exceptions through a weekly commercial cadence.',
          );
        await page
          .locator('#init-context-problem')
          .fill(
            'Regional discount leakage obscures profitable growth and weakens accountability for realized margin.',
          );
        await page
          .locator('#init-value-logic')
          .fill(
            'A governed one-point margin recovery creates recurring gross-margin uplift after adoption and data-quality controls are in place.',
          );
        await page
          .locator('#init-deps')
          .fill('Requires reliable price waterfall data and regional sales-owner adoption.');
        await page.getByRole('button', { name: 'Go to next step' }).click();
        ...
        await expect(
          page.getByRole('heading', { name: temporaryInitiativeName }).first(),
        ).toBeVisible();
        ...
        results.temporaryInitiative = {
          name: temporaryInitiativeName,
          code: temporaryInitiativeCode,
          created: true,
        };
    ...
    await scenario(
      'Temporary initiative financial scope and baseline',
      'Configure the new initiative, save its original operating baseline, and prove persistence after a browser reload.',
      async () => {
        ...
        const costToggle = page
          .locator('label')
          .filter({ hasText: 'Software / Licenses' })
        ...
        await page
          .getByLabel('Initiative baseline Annual Gross Margin Baseline')
          .fill('1350000.0000');
        ...
        console.log('[acme-guide] baseline control state', {
          ...
          revenue: await page
            .getByLabel('Initiative baseline Annual Revenue Baseline')
            .inputValue(),
          grossMargin: await page
            .getByLabel('Initiative baseline Annual Gross Margin Baseline')
            .inputValue(),
        ...
        await expect(page.getByLabel('Initiative baseline Annual Revenue Baseline')).toHaveValue(
          '3000000.0000',
        );
        await expect(
          page.getByLabel('Initiative baseline Annual Gross Margin Baseline'),
        ).toHaveValue('1350000.0000');
      },
    );

    await scenario(
      'Temporary initiative benefit and cost lines',
      'Build an auditable example value case with a named benefit and a separately phased recurring implementation cost.',
      async () => {
        expect(temporaryInitiativePath).toBeTruthy();
        await openRoute(page, temporaryInitiativePath);
        const initialGrid = waitForInitiativeFinancialGrid(page, temporaryInitiativePath);
        ...
        await page.reload();
        const persistedValueCaseGrid = waitForInitiativeFinancialGrid(
          page,
          temporaryInitiativePath,
        );
        await page.getByRole('button', { name: 'Financials', exact: true }).click();
    ...
    await scenario(
      'Saturday meeting command center',
      'Create a Saturday steering series, build an initiative-backed agenda, capture notes, attach a decision and risk to that agenda, generate/review AI minutes, complete the session, and remove the temporary series.',
      async () => {
        ...
        await page
          ...
          .fill('Weekly ACME steering review of value, milestones, decisions, and risks.');
        await page
          .locator('fieldset')
        ...
        const notes =
          'ENT-005 remains on the FY28 value path. Finance will confirm the benefit owner. Architecture will name a rollback owner before migration. The committee will retain the Saturday checkpoint until migration risk is green.';
        await page
        ...
        await page
          ...
          .fill('Retain Saturday checkpoint until migration risk is green');
        ...
        await page
          .getByPlaceholder('Capture a new action item...')
          .fill('Migration rollback ownership is not yet confirmed');
        await page.getByRole('button', { name: 'Add risk' }).click();
        await expect(
          page.getByText('Retain Saturday checkpoint until migration risk is green').first(),
        ).toBeVisible();
        await expect(
          page.getByText('Migration rollback ownership is not yet confirmed').first(),
        ).toBeVisible();
        ...
        const minutesSave = page.waitForResponse(
          (response) =>
        ...
        results.temporaryMeeting = {
          name,
          day: 'Saturday',
          completed: true,
          decisionAttached: true,
          riskAttached: true,
        };
        await openRoute(page, '/admin');
    ...
    await scenario(
      'Deterministic temporary initiative cleanup',
      'Return ACME to its canonical ten-initiative demo state entirely through the admin browser workflow.',
      async () => {
        ...
        await page.getByRole('button', { name: 'Open Data Cleanup admin tab' }).click();
        await page
          .getByRole('button', { name: `Select ${temporaryInitiativeName} for deletion` })
          .click();
        ...
        await openRoute(page, '/meetings');
        await expect(
          page.getByText(results.temporaryMeeting?.name || 'impossible-name'),
        ).toHaveCount(0);
    ...
    writeFileSync(
      resolve(repo, 'scratch/issue-447/acme-guide-full.json'),
